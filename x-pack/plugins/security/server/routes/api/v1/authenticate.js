...
export function initAuthenticateApi(server) {
  ...
  server.route({
    method: 'POST',
    path: '/api/security/v1/generateTicket',
    config: {
      auth: false,
      validate: {
        payload: Joi.object({
    ...
    async handler(request) {
      ...
      // console.log(h)
      try {
        request.loginAttempt().setCredentials(username, password, true);
        const authenticationResult = await server.plugins.security.authenticate(request);
        ...
        return {data:{
                      ticket:authenticationResult.ticket
                    },
                success:true};

        // return authenticationResult.ticket;
      } catch(err) {
        throw wrapError(err);
      }
    }
