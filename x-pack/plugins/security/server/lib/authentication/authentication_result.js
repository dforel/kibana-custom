...
export class AuthenticationResult {
  ...
  get state() {
    ...
    /**
   * isTicketLogin associated with the authenticated user (only available for `succeeded`
   * ).
   * @type {?Object}
   */
  get isTicketLogin() {
    return this._isTicketLogin;
  }
 ...
 /**
   ...
   * @private
   */
  constructor(status, { user, state, error, redirectURL, isTicketLogin, ticket } = {}) {
    this._status = status;
    this._user = user;
    this._state = state;
    this._isTicketLogin = isTicketLogin;
    this._ticket = ticket;
    this._error = error;
   ...
   * @returns {AuthenticationResult}
   */
  static succeeded(user, state, isTicketLogin, ticket) {
    if (!user) {
    ...
    return new AuthenticationResult(
      ...
      { user, state, isTicketLogin, ticket}
    );
  }
