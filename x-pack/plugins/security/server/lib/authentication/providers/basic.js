...
export class BasicAuthenticationProvider {
  ...
  async authenticate(request, state) {
    ...
    if (authenticationResult.notHandled() && state) {
      ...
      // 如果带有认证用的ticket,那就删除这个参数再重定向,防止出问题
      var path = request.url.path;
      if(path.indexOf("?ticket=")>=0){
        var path = urlDelP(path,"ticket");
      }

      // If we couldn't handle authentication let's redirect user to the login page. 
      const nextURL = encodeURIComponent(`${request.getBasePath()}${path}`); 
  ...
  async _authenticateViaLoginAttempt(request) {
    ...
    try {
      ...
      const { username, password, isTicketLogin } = credentials;
      ...
      return AuthenticationResult.succeeded(user, { authorization: request.headers.authorization } , isTicketLogin );
    } catch(err) {
