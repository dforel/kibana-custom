...
class Authenticator {
   ...
   */
  constructor(server, authScope, session, authorizationMode, ticket) {
    this._server = server;
  ...
  async authenticate(request) {
    ...
    var existingSession = this._ticket.get(request);

    // console.log(existingSession);
    // const existingSession = await this._session.get(request);
    if(!existingSession){ 
      existingSession = await this._session.get(request); 
    }
    ...
    for (const [providerType, provider] of this._providerIterator(existingSession)) {
      ...
      if (ownsSession || authenticationResult.shouldUpdateState()) {
        ...
        if (authenticationResult.shouldClearState() || (
          ...
          if(authenticationResult.isTicketLogin ){ 
            var ticket =  this._ticket.set(request,{ state: authenticationResult.state, provider: providerType }) 
            authenticationResult.ticket = ticket; 
          }
      ...
      if (authenticationResult.succeeded()) {
        ...
        null,
        authenticationResult.isTicketLogin,
...
export async function initAuthenticator(server, authorizationMode) {
  ...
  const ticket = new Ticket(server);

  const authScope = new AuthScopeService();
  const authenticator = new Authenticator(server, authScope, session, authorizationMode, ticket);
